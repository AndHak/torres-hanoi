              ...
              {pasos.map((paso, indice) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className="font-bold">MOV {indice.toString().padStart(2, "0")}</span>
                      <span>
                        g:{paso.g} h:{paso.h.toFixed(0)} f:{paso.f.toFixed(0)}
                      </span>
                    </div>
                    <p className={"truncate font-sans font-bold " + (estaActivo ? "text-white" : "text-slate-600")}>
                      {paso.movimiento}
                    </p>
                  </button>
                );
      ...
      {pasos && (
        <div className="p-4 border-t border-slate-100 bg-white shrink-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Resultados</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
              <span className="text-[9px] text-slate-400 uppercase font-bold block">Nodos expandidos</span>
              <p className="text-sm font-bold text-slate-800">{nodosExpandidos}</p>
            </div>
            <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
              <span className="text-[9px] text-slate-400 uppercase font-bold block">Movimientos</span>
